...
public class Car extends Transport{
	...
	float TotalFareWithTax() {
		...
		setTotalFareWithTax ((float) ((getFare()*getSeatCapacity()*getDistance())+0.02*(getFare()*getSeatCapacity()*getDistance())));
	...
	float Discount() {
		...
		if (getTotalFareWithTax() > 100) {
			setDiscount((float) (getTotalFareWithTax()*0.10)); 
		}
		else {
			setDiscount(0);
		}
	...
	public void bookCar() {
		//User Input of Transport ID, Name, Seat capacity, Distance, and Fare to book the CAR
